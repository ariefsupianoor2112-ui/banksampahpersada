...
@section('title', 'Pengajuan Setoran')

@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-[#1b4332]">Pengajuan Setoran</h1>
        <p class="text-slate-500 text-sm mt-1">Setoran yang diajukan sendiri oleh nasabah lewat menu "Ajukan Setoran".</p>
    </div>

    <!-- Filter status -->
    <div class="flex flex-wrap gap-2 mb-5">
        @foreach(['pending' => 'Menunggu', 'approved' => 'Disetujui', 'rejected' => 'Ditolak', 'semua' => 'Semua'] as $key => $label)
            <a href="{{ route('admin.setoran.index', ['status' => $key]) }}"
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($setoran as $s)
                    <tr>
                        <td class="py-3 px-4 text-slate-500 whitespace-nowrap">{{ $s->created_at->format('d M Y, H:i') }}</td>
                        <td class="py-3 px-4 font-medium">{{ $s->user->name }}</td>
                        <td class="py-3 px-4">{{ $s->jenisSampah->nama_sampah ?? '—' }}</td>
                        <td class="py-3 px-4 text-right">{{ number_format($s->berat_kg, 2, ',', '.') }} kg</td>
                        <td class="py-3 px-4 text-right font-semibold">Rp {{ number_format($s->total, 0, ',', '.') }}</td>
                        <td class="py-3 px-4">
                            @if($s->status === 'pending')
                        ...
                        <td class="py-3 px-4">
                            @if($s->status === 'pending')
                                <div class="flex items-center justify-center gap-2">
                                    <form method="POST" action="{{ route('admin.setoran.approve', $s) }}">
                                        @csrf
                                    ...
                                    <form method="POST" action="{{ route('admin.setoran.reject', $s) }}" onsubmit="return confirm('Tolak setoran ini?')">
                                        @csrf
                ...
                @empty
                    <tr><td colspan="7" class="py-8 text-center text-slate-400">Tidak ada pengajuan setoran.</td></tr>
                @endforelse
            </tbody>
        </table>
        <div class="px-6 py-4">{{ $setoran->links() }}</div>
    </div>
@endsection
