...
@section('content')
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-7 mt-6">
        ...
        <p class="text-slate-500 text-sm mt-1 mb-6">Gunakan ID Nasabah dan kata sandi yang terdaftar.</p>
        <form action="{{ url('/login') }}" method="POST" class="space-y-4">
            ...
            @endif
            <div>
                <label class="block text-xs font-bold uppercase tracking-wide text-slate-500 mb-1.5">ID Nasabah</label>
                <input type="text" name="kode" required autofocus value="{{ old('kode') }}"
            ...
            <label class="flex items-center gap-2 text-sm text-slate-500">
                <input type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}
                    class="rounded border-slate-300 text-[#1b4332] focus:ring-[#1b4332]">
                Ingat saya
            </label>
            <button type="submit"
